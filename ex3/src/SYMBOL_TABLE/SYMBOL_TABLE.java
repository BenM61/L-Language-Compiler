...
public class SYMBOL_TABLE {
	...
	public TYPE findName(String name) {
		int normalPrevtopInd = findPrevtopIndex(name);
		int inheritancePrevtopInd = inheritFindPrevtopIndex(name);
		TYPE res;

		if (normalPrevtopInd >= inheritancePrevtopInd) {
			res = find(name);
			System.out.format("closest dec of %s is via normal find, %d\n", name, normalPrevtopInd);
		} else {
			res = inheritFind(name);
			System.out.format("closest dec of %s is via inherited find, %d\n", name, inheritancePrevtopInd);
		}

		return res;
	}

	public int findPrevtopIndex(String name) {
		SYMBOL_TABLE_ENTRY e;
	...
	public TYPE inheritFind(String name) {
		String className = inClassScope();

		if (className == null) {
			return null;
		}

		TYPE res = findInClassScope(name);
		...
		TYPE classType = find(className);
		if (!classType.isClass() || !classType.name.equals(className)) {
			System.out.format(">> ERROR in findInherit--- class Name=%s, found Name= %s\n", className, classType.name);
			return null;
		}

		TYPE_CLASS parentClassType = ((TYPE_CLASS) classType).father;

		while (parentClassType != null) {
			System.out.format(">> now searching for %s in class %s\n", name, parentClassType.name);

			// search in parent fields
			for (AST_ARG_LIST it = parentClassType.data_members; it != null; it = it.tail) {
				AST_ARG curr = it.head;
				if (curr.id.equals(name)) {
					return find(curr.t.typeName);
				}
			}

			// move one step up the hirearchy
			classType = parentClassType;
			if (!classType.isClass() || !classType.name.equals(className)) {
				System.out.format(">> ERROR in findInherit--- class Name=%s, found Name= %s\n", className, classType.name);
				return null;
			}

			parentClassType = ((TYPE_CLASS) classType).father;
		}

		return null;
	}

	public int inheritFindPrevtopIndex(String name) {
		String className = inClassScope();
		...
		int res = findInClassScopePrevtopIndex(name);
		...
		while (parentClassType != null) {
			...
			for (AST_ARG_LIST it = parentClassType.data_members; it != null; it = it.tail) {
				AST_ARG curr = it.head;
				if (curr.id.equals(name)) {
					return findPrevtopIndex(parentClassType.name);
				}
			}
	...
	public int findInClassScopePrevtopIndex(String name) {
		SYMBOL_TABLE_ENTRY curr = top;
		int res = -1;

		while (!curr.name.startsWith("SCOPE-BOUNDARY-class")) {
			if (curr.name.equals(name)) {
				res = curr.prevtop_index;
			}
			curr = curr.prevtop;
		}
		return res;
		...
		while (a.name != null) {
			if (a.type.isFunc()) {
				...
				for (AST_EXPLIST it = params; it != null; it = it.tail) {
					...
					System.out.format("now matching %s %s\n", curr.name, t.name);
					if (t.name == curr.name) {
						System.out.format("V\n");
						l = l.tail;
						continue;
